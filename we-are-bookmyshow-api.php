...
$url = 'https://we-are.bookmyshow.com/?format=json';
...
if( $decodeJsonData["success"] == 1 ) {
    ...
    $bookmyshowPostsId = $decodeJsonData["payload"]["collection"]["sections"][1]["postListMetadata"]["postIds"];
    ...
    for($i=0; $i<count($postedByUsers); $i++) {
        ...
        $mainGlobal[$i]["url"] = "https://we-are.bookmyshow.com/" . $globalArr[$i]["slug"] . "-" . $globalArr[$i]["id"];
    ...
    $bookmyshowPostsJson = $mainGlobal;
    
    echo '[{ "response" : '; 
    echo '"<br/>';   
    foreach($bookmyshowPostsJson as $key => $value) {
    	$num2++;
